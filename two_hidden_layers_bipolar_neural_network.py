...
class TwoHiddenLayersBipolarNeuralNetwork:
    def __init__(self, X, T, H1, H2, alpha):
        self.X = X
        self.T = T
        self.num_hidden1 = H1
        self.num_hidden2 = H2
        ...
        print("weights01")
        print(self.weights01)
        print(self.weights01.shape)
        print("weights12")
        print(self.weights12)
        print(self.weights12.shape)
        print("weights23")
        print(self.weights23)
        print(self.weights23.shape)
    ...
    def back_propigation(self):
        ...
        #dJ/dZ3 = dJ/dY x dY/dZ3
        #grad of J wrt Y = dJ/dY(||T-Y|||^2) = 2(T-Y)
        #grad of Y wrt Z2 = dY/dZ3(1/(1+np.exp(-z))) = 1+np.exp(-z)) * (1 - 1+np.exp(-z)))
        dJdZ3 = 2*(self.T - self.Y) * bipolar_sigmoid(self.Y, derivative=True)
        ...
        dJdW23 = np.dot(self.activation2.T, dJdZ3)
        dJdB23 = dJdZ3

        #calculate gradient of loss with respect to net input for layer 1 (dJ/dZ1)
        #dJ/dZ2 = f'(Z2) x (W23)T x dJ/dZ3
        dJdZ2 = bipolar_sigmoid(self.activation2, derivative=True) * self.weights23.T * dJdZ3
        ...
        dJdZ1 = bipolar_sigmoid(self.activation1, derivative=True) * self.weights12 * dJdZ2
        #dJ/dW01 = (A1)T x dJdZ1
        ...
        self.weights23 = self.weights23 + self.learning_rate * dJdW23
        self.bias23 = self.bias23 + self.learning_rate * dJdB23
